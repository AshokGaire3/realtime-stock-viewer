...
**forecast accuracy evaluation** (`PriceBar`, `ForecastRun`, `ForecastPoint`). The tables are
created at startup, but **no endpoints read or write them yet** — the schema is groundwork,
not a working feature. Likewise the **AI tutor** and **indicator explainers** are planned, not
implemented; `ANTHROPIC_API_KEY` is currently unused.
